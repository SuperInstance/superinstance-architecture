...
This document defines the system architecture for SuperInstance — a voice-controlled, self-assembling distributed system for commercial fishing vessels and remote environments. Conceived by Casey Digennaro in Sitka, Alaska, the system integrates 3,200+ repositories partitioned among four sovereign software agents ("Vessels") spanning ESP32 microcontrollers, Raspberry Pi coordinators, NVIDIA Jetson inference nodes, and cloud LLM services via Starlink ^1^. Scope covers system design, protocols, hardware tiers, agent identity, safety, voice pipeline, and roadmap. Excluded: installation guides, API references, and operational playbooks. Audience: systems architects, edge AI engineers, distributed systems researchers, and marine technology integrators.

## System at a Glance

SuperInstance decomposes every application into "rooms" — simultaneously physical locations on the vessel (bridge, engine room, backdeck), virtual knowledge spaces in PLATO, and computational zones defined by DDS domains. An agent entering a room relocates to the nearest Jetson node, joins that DDS domain, and loads the room's PLATO context as an atomic transition ^2^. Voice commands enter through CopilotKit, which translates utterances into structured tool calls mapped to device capabilities ^2^. Table 1 maps the five governing principles to their system manifestations.

| Principle | System Manifestation | Binding Constraint |
|:---|:---|:---|
| Rooms as universal context | MQTT topics `/vessel/{room}/{device}/{metric}`; DDS domains per room; PLATO knowledge rooms with portal-based cross-room access ^2^| Context membership is explicit; every state object carries a room scope |
| Vessels as self-sovereign identities | `did:key` Decentralized Identifiers; W3C Verifiable Credential capability tokens; git-agent lifecycle as attestation ^2^ ^3^| No centralized identity provider exists at sea; offline verification is mandatory |
| Reflex precedes deliberation | `pincher` reflex engine <1 ms; ESP32 wake word at 18–22 ms inference never leaves the MCU ^2^ ^4^| Safety-critical commands cannot tolerate LLM latency (200–2,000 ms) ^5^|
| Self-assembly as default | mDNS same-room; gossip-based SWIM vessel-wide; Kademlia DHT global; K3s auto-registration ^2^| Manual configuration infeasible on a vessel with 20+ heterogeneous nodes |
| Voice as primary control plane | CopilotKit `useCopilotAction` hooks; A2UI dynamic React rendering; tool schemas per hardware node ^2^| Eliminates custom NLU firmware; any device exposing tools becomes voice-controllable |
...
|:---|:---|:---|:---|:---|:---|
| 0 — Edge Sensor | ESP32-S3 | Dual-core Xtensa @ 240 MHz, TFLite Micro INT8 | ~0.5 W ^6^| Wake-word inference 18–22 ms ^4^| $3–8 ^7^|
| 1 — Coordinator | Raspberry Pi 5 8GB | Quad-core Cortex-A76 @ 2.4 GHz, K3s | 5–7 W ^8^| STT 1.4 s (5 s utterance) ^9^| ~$80 |
| 2 — AI Inference | Jetson Orin Nano 8GB | 1024 CUDA cores, 67 TOPS INT8 ^2^| 7–15 W ^2^| LLM ~28 tok/s (Llama 3.2 3B) ^4^| $259–499 |
| 3 — Cloud Backhaul | Starlink + Cloud APIs | Remote GPU (GPT-4o, Claude) | 25–75 W (terminal) | 25–50 ms RTT median ^1^| $75–120/mo |
...
The architecture is validated against five quantitative targets. **Voice-to-action** must remain under 3,000 ms end-to-end (P0); measured median is 1,980 ms ^5^ ^1^. **Reflex response**: hardware path (ESP32 GPIO) at <1 ms (P0), software path (Pincher regex plus embedding) at <700 ms (P0). **Auto-join** (power-on to mesh participation) must complete in <60 seconds (P1). **Offline autonomy** requires 24 hours of full local operation (P1). **Jetson throughput** must exceed 25 tok/s (P1); the Orin Nano achieves ~28 tok/s ^4^.
...
| Reflex precedes deliberation | `pincher` regex+embedding reflex engine for safety commands; ESP32 wake word never leaves MCU | Safety-critical commands cannot tolerate LLM latency variance (200–2,000 ms) ^5^|
...
|--------|-------|-------------------|
| Total repositories | 3,200+ | GitHub organization count ^16^|
| Cataloged repositories | 2,000 | `CATALOG.md` (8,262 lines) ^17^|
...
| 5. cudaclaw | PTX kernel binary | Persistent kernel deployment, memory-resident execution, fleet distribution | Running GPU kernel (resident in VRAM) | RTX 4050, Jetson Orin ^4^|
...
**PULL** fetches the latest repository state, including three critical files: `CHARTER.md` (the agent's purpose and operational constraints), `STATE` (current status and context), and `TASK-BOARD` (prioritized work queue). These files function as the agent's runtime state — there is no external database or process memory required for agent persistence ^27^.
...
|-----------|---------------|----------|---------------|
| constraint-theory-core | Production | crates.io v1.0.1, 83 tests, zero deps, PyO3 + WASM bindings ^19^| None identified |
| constraint-theory-python | Production | PyPI published, installable via pip ^20^| None identified |
| pincher | Active development | 57 commits, CI/CD, doc suite, cargo installable ^24^| Limited integration tests with upstream layers |
| plato-sdk | Active development | PyPI published, Python SDK for PLATO rooms ^20^| Documentation fragmented across repos |
| cuda-oxide | Active development | Flux→MIR→PTX pipeline functional ^26^| No distributed GPU runtime in production use |
| cudaclaw | Experimental | 6 kernels on RTX 4050 demonstrated ^4^| No Jetson deployment evidence; no persistent kernel benchmarks |
| cocapn fleet | Experimental | Active PR work, 3 GitHub stars ^16^| No cross-repo integration test suite |
| agent-riff-v4 | Experimental | Self-bootstrapping spec generation ^16^| 4th-generation rewrite suggests instability |
| Hardware stack (Jetson/ESP32) | Experimental | Code exists, limited CI for physical hardware ^23^| Hardware-in-the-loop testing gap |
| Fleet observability | Early | holodeck-session-manager exists ^16^| No Prometheus/Grafana integration; no metrics pipeline |
...
The ESP32-S3 is the acoustic front-line. A dual-core Xtensa LX7 at 240 MHz with 512 KB SRAM and 8 MB PSRAM runs TensorFlow Lite Micro inference for wake word detection ^7^. The pipeline — I2S sampling at 16 kHz, MFCC extraction across 40 mel bins, and an 80 KB INT8 CNN — completes single-window inference in 18–22 ms, with posterior smoothing over three windows suppressing false activations ^4^. After wake word detection, the ESP32 activates WiFi, publishes the command via MQTT to the Tier 1 broker, and returns to deep sleep. Power spans three regimes: 0.8 mA in light sleep, 35 mA during the ~25 ms inference burst, and 160–260 mA with WiFi active ^6^. A 2000 mAh cell supports approximately four months of continuous listening ^4^. Unit cost is $3–8 ^7^.

#### 4.1.2 Tier 1 — Raspberry Pi 5 8GB: Coordination and Lightweight Inference

The Raspberry Pi 5 8GB serves as the vessel's edge coordinator, hosting K3s, the MQTT broker for ESP32 telemetry aggregation, and lightweight inference fallback ^10^. On the Pi 5, `whisper.cpp` with `tiny.en` runs at 3.5x real-time, transcribing a 5-second utterance in ~1.4 seconds ^9^. TinyLlama 1.1B (Q4_K_M) generates 12–18 tok/s for simple intent parsing ^8^. An NVMe SSD via M.2 HAT reduces model load time from ~18 seconds (microSD) to ~3 seconds ^28^. Active cooling is mandatory: sustained inference without a heatsink triggers 20–30% thermal throttling ^8^. Power draw is 5–7 W active, 2–3 W idle. Cost is ~$80.

#### 4.1.3 Tier 2 — NVIDIA Jetson Orin Nano: AI Inference and Vision

The Jetson Orin Nano 8GB Super delivers 67 TOPS (INT8) at 7–15 W configurable TDP ^2^. TensorRT-LLM achieves ~28 tok/s with Llama 3.2 3B (Q4_K_M, 2.0 GB) and ~15 tok/s with Llama 3.1 8B, both fitting within 8 GB unified LPDDR5 ^4^ ^12^. DeepStream 7.1 provides GPU-accelerated multi-stream video analytics supporting up to 30 concurrent camera feeds ^29^— this is where backdeck camera processing (object detection, gear tracking, safety monitoring) executes in real time. The Jetson runs as a K3s worker node with the NVIDIA Container Runtime ^28^. Cost is $259–499.

#### 4.1.4 Tier 3 — Starlink and Cloud: Satellite Connectivity

Tier 3 combines satellite connectivity with remote compute. Starlink Standard delivers 100–400 Mbps downstream with median RTT of 25–50 ms (99th percentile <65 ms) ^1^. Satellite handoffs every ~15 seconds can inject 100–500 ms spikes, but packet loss stays below 1% under clear conditions ^29^. Over this link, GPT-4o Realtime API achieves ~320 ms median end-to-end latency for native audio-to-audio conversation ^14^. When Starlink is unavailable, the orchestration falls back to Tier 2 (Jetson) for local inference. Service cost is $75–120 per month.
...
| **GPU / AI** | Vector SIMD | VideoCore VII (~2 TOPS est.) | 1024 CUDA cores, 67 TOPS INT8 ^2^| Cloud GPU (varies) |
| **RAM** | 512 KB SRAM + 8 MB PSRAM | 8 GB LPDDR4X | 8 GB LPDDR5 unified ^12^| Elastic |
| **Storage** | 16 MB Flash | NVMe SSD (via HAT) | NVMe SSD (M.2) | Elastic |
| **Active Power** | 160–260 mA (~0.5 W) ^6^| 5–7 W ^8^| 7–15 W ^2^| 25–75 W (terminal) |
| **Deep Sleep** | 0.8 mA ^4^| 2–3 W idle | 3–5 W idle | N/A |
| **Network** | WiFi 4, BLE 5.0 | WiFi 5, GigE | WiFi 5, GigE | 100–400 Mbps ^1^|
| **Unit Cost** | $3–8 ^7^| ~$80 | $259–499 ^2^| $75–120/mo |
| **OS** | FreeRTOS / ESP-IDF | Raspberry Pi OS 64-bit | JetPack 6.1 (Ubuntu) ^12^| N/A |
...
Protocol selection is driven by payload size, latency tolerance, and device capability. **ESP32 → RPi** uses MQTT (QoS 1 for commands, QoS 0 for telemetry): its 2-byte minimum header and broker-based architecture minimize ESP32 energy use and handle intermittent WiFi gracefully, with topic namespacing by room (`/vessel/bridge/commands`, `/vessel/engine/telemetry`). **RPi → Jetson** uses gRPC with Protocol Buffers, delivering 50–70% lower latency and 5–10x smaller payloads than REST/JSON ^30^ ^31^; bidirectional streaming supports chunked audio for STT and token streaming from the LLM. **Jetson → Cloud** uses HTTP/2 with persistent connection pooling, eliminating the ~100–300 ms TLS handshake penalty that repeated negotiation would incur over Starlink ^29^; WebSocket is used specifically for GPT-4o Realtime API streaming. **Fallback** for any tier pair falls back to MQTT QoS 1 with retained messages, allowing state recovery on reconnection.
...
|---|---|---|---|---|
| Wake word detection | Tier 0 (ESP32-S3) | TFLite Micro, 80 KB INT8 | 18–22 ms ^4^| None (always local) |
| STT (command) | Tier 1 (RPi 5) | whisper.cpp tiny.en | ~1.4x real-time ^9^| Tier 2 (faster-whisper) |
| STT (long-form) | Tier 2 (Jetson) | faster-whisper medium | Sub-1s ^32^| Tier 1 (slower) |
| LLM (simple intent) | Tier 1 (RPi 5) | TinyLlama 1.1B Q4 | 12–18 tok/s ^8^| Tier 2 (Llama 3.2 3B) |
| LLM (complex reasoning) | Tier 2 (Jetson) | Llama 3.2 3B Q4 | ~28 tok/s ^4^| Tier 3 (GPT-4o) |
| LLM (analysis, large ctx) | Tier 3 (Cloud) | GPT-4o Realtime | ~320 ms e2e ^14^| Tier 2 (Llama 3.1 8B) |
| Text-to-speech | Tier 1 or 2 | Piper / Kokoro | 50–200 ms ^15^| Tier 1 (Piper on CPU) |
| Vision (backdeck cam) | Tier 2 (Jetson) | DeepStream + TensorRT | Real-time ^12^| Alert-only (no AI) |
...
|:---|:---|:---|:---|:---|:---|:---|
| **Forgemaster** | 330 ^2^| Compilation, math, GPU kernels | FLUX bytecode, PTX, Rust crates | Jetson Orin Nano (Tier 2) | `pincher`, `constraint-theory-core`, `cudaclaw` (crates.io) ^2^| Read-only: cached kernels on Jetson; no new compilation until recovery |
| **CCC** | 116 ^2^| Human interface, web UI, CopilotKit | React components, GraphQL, AG-UI protocol | RPi 5 (Tier 1), Cloud (Tier 3) | `@superinstance/cocapn-lighthouse` (npm) ^2^| Static dashboard fallback; voice commands queue for replay |
| **JetsonClaw1** | 76 ^2^| Hardware edge, sensors, firmware | MQTT topics, DeepStream pipelines, NMEA 0183 | ESP32 (Tier 0), Jetson (Tier 2) | `plato-edge`, `tile-refiner` (PyPI) ^2^| Sensor data buffered locally; reflex commands via `pincher` cache |
| **Oracle1** | 43 ^2^| Infrastructure, search, coordination | REST API, PLATO SDK, lighthouse gossip | RPi 5 (Tier 1), Cloud (Tier 3) | `plato-sdk`, `fleet-homunculus` (PyPI) ^2^| DHT cache survives outage; gossip maintains partial discovery |
...
Each Vessel's identity is anchored in a `did:key` document using the Ed25519 format. The `did:key` method is selected over `did:web` (requires DNS) and `did:ethr` (requires blockchain) because it supports fully offline verification during Starlink outages ^2^.
...
Each Vessel publishes capabilities as JSON-LD to three discovery layers. Local advertisement uses mDNS (RFC 6762) within the broadcast domain for room-level discovery without infrastructure ^2^. Vessel-wide uses SIR (Susceptible-Infectious-Recovered) gossip for probabilistic propagation balancing bandwidth against reliability. Global uses a libp2p Kademlia DHT providing O(log n) lookups ^2^. This three-tier approach, confirmed by cross-verification of distributed-patterns and ecosystem-map analyses, is optimal for maritime deployments with intermittent connectivity.

#### 5.3.2 Service Discovery

Discovery operates at three scopes. Local uses mDNS/Bonjour via Avahi with capability summaries in TXT fields. Vessel-wide uses gossip-based SWIM membership (HashiCorp Memberlist style) with suspicion tables for peer liveness. Global uses Kademlia DHT bootstrapped from Oracle1's lighthouse ^2^. On boot, a Vessel performs all three in parallel: mDNS for immediate neighbors (sub-100 ms), gossip for fleet convergence (1–5 seconds), and DHT for global context (25–50 ms per Starlink hop) ^1^.
...
Two consensus mechanisms operate by trust boundary. Within a room — physically co-located, administratively trusted devices — Raft provides crash-fault tolerance with O(n) complexity ^2^. Each room forms a small Raft cluster (3–5 nodes: ESP32, RPi coordinator, Jetson worker) electing a leader for local ordering. For cross-Vessel decisions — administratively distinct participants — a ternary voting protocol uses the `{-1, 0, +1}` logic native to SuperInstance's `open-parallel` layer. Each Vessel casts `-1` (disagree), `0` (abstain), or `+1` (agree); consensus requires `+1` from `2f+1` of `3f+1` voters, tolerating `f` Byzantine faults ^2^. Ternary values require 2 bits versus 32+ bits for floating-point scores, with the 16x memory bandwidth savings from `open-parallel` applying equally to consensus state ^2^.
...
The voice pipeline is a cascade of six stages, each with defined latency targets. An ESP32-S3 node in each physical room (bridge, engine room, back deck, hold) runs a TensorFlow Lite Micro wake-word model that infers in 18–22 ms on a 240 MHz dual-core Xtensa LX7 ^7^ ^6^. Upon detection, the node activates Wi-Fi and streams 16 kHz mono audio over MQTT to the Raspberry Pi 5 coordinator, where VAD (Silero, <1 ms per 30 ms frame) determines speech endpointing ^5^. The audio buffer is then passed to whisper.cpp (tiny.en model) running on the Pi 5 at 3.5x real-time, yielding a text transcript in approximately 300 ms for a typical 5-second utterance ^9^.

Context assembly combines the transcript with the current room state (which ESP32 node triggered), the set of capabilities registered by Vessels in that room, and the last three turns of conversation history from Redis. The assembled prompt is dispatched to the LLM — DeepSeek V4 Flash via Starlink for complex reasoning, or Llama 3.2 3B on the Jetson at ~28 tok/s for local, offline-capable inference ^4^ ^12^. The LLM does not emit free-form prose; it generates structured tool calls in JSON that map to `useCopilotAction` schemas. If the command is "dim the bridge lights to thirty percent," the model emits `{"action": "set_light", "vessel": "JetsonClaw1", "room": "bridge", "brightness": 30}`.

Response synthesis reverses the path: tool results are injected back into the LLM context, a natural-language confirmation is generated, and Piper or Kokoro TTS renders it to audio on the Pi 5 in roughly 100 ms ^15^ ^33^. The audio is published over MQTT to the originating ESP32 node for I2S playback. The full pipeline, illustrated in Figure 6.2, completes in approximately 1.5 seconds under normal conditions, with VAD endpointing (500 ms typical) and LLM processing (500 ms typical) consuming two-thirds of the budget.
...
|---|---|---|---|
| Voice capture (audio buffer) | 40 ms | 16 kHz I2S ring buffer on ESP32-S3 | ^5^|
| VAD endpointing (silence detection) | 500 ms | Silero VAD, tuned for marine ambient noise | ^5^|
| STT (whisper.cpp tiny.en) | 300 ms | 3.5x real-time on Raspberry Pi 5 | ^9^|
| Network transit (MQTT + GraphQL) | 10–30 ms | Local Wi-Fi, negligible vs. Starlink | ^29^|
| LLM processing (cloud via Starlink) | 500 ms | DeepSeek V4 Flash; 25–50 ms Starlink RTT | ^1^ ^12^|
| TTS (Piper / Kokoro) | 100 ms | CPU-rendered on Pi 5 | ^15^|
| Audio playback buffer | 40 ms | I2S output on ESP32 | ^5^|
...
The CopilotKit runtime sits on the Raspberry Pi 5 coordinator alongside Mosquitto (MQTT broker), K3s control plane, and whisper.cpp ^10^. GraphQL requests that target Vessel capabilities are translated to gRPC/Protobuf calls into the Rust backend, which yields 50–70% lower latency and 5–10x smaller payloads than REST/JSON ^30^ ^31^. A2UI-rendered components — live camera feeds, sensor gauges, diagnostic panels — are defined in the frontend as `useCoagentStateRender` registrations; when the agent enters a state such as `displaying_camera_feed`, the corresponding React component mounts with the room-appropriate video stream sourced from Oracle1's DeepStream pipeline. This architecture means the fleet dashboard is not a static page but a dynamically generated interface that adapts to voice context, physical location, and current operational state.
...
Speed demands simplicity. A sub-millisecond response leaves no time for neural network inference or context evaluation. The ESP32 path is deterministic: interrupt triggered → lookup table consulted → GPIO output set. Intelligence requires complexity: a command like "adjust course to avoid that vessel" demands sensor fusion and planning spanning thousands of tokens. The Jetson achieves 28 tok/s for Llama 3.2 3B ^4^, but even minimal prompts take hundreds of milliseconds.
...
The ESP32 tier uses GPIO interrupts with ISRs stored in internal RAM (`IRAM_ATTR`) to avoid flash cache latency ^34^. The RPi tier leverages Pincher's vector database at 57 commits, Rust 76.6% ^25^. The Jetson tier routes through TensorRT-LLM or Starlink at 25–50 ms RTT ^1^.

| Tier | Hardware | Trigger Type | Min Latency | Max Latency | Typical | Budget Source |
|---|---|---|---|---|---|---|
| Reflex (bare-metal) | ESP32-S3 | GPIO interrupt | 0.1 ms | 1.0 ms | 0.5 ms | ISR in IRAM ^34^|
| Reflex (pattern-match) | RPi 5 | Pincher regex + embedding | 1.0 ms | 10 ms | 3.0 ms | Vector DB, no LLM path ^25^|
| Deliberative (local) | Jetson Orin Nano | LLM (3B model) | 700 ms | 2,000 ms | 1,200 ms | TensorRT-LLM at 28 tok/s ^4^|
| Deliberative (cloud) | Starlink + API | GPT-4o / Claude | 725 ms | 3,000 ms | 1,500 ms | 25–50 ms RTT + API TTFT ^1^|
...
The `deviceType` field maps to the four hardware tiers from Chapter 4, letting recipients infer baseline capabilities without negotiation. The `vesselId` anchors the node to a Vessel identity, which serves as the trust root for subsequent authentication ^20^.
...
The state machine is implemented in the `construct-coordination` crate (80 commits, highest activity in the coordination family) ^20^. Each transition is logged to the local append-only event log. Timeout values along the critical path from DISCOVERING through ACTIVE sum to 60 seconds, enforcing the boot-to-participation target ^16^.
...
|---|---|---|---|
| Round-trip time (RTT) | 25–50 ms ^1^| < 65 ms normal; 100–500 ms during handoff ^29^| Base network latency for every API request |
| Jitter | 5–20 ms ^33^| Up to 50 ms in rough seas / antenna motion | Manageable with HTTP/2 multiplexing and retry |
| Satellite handoff spike | 15–100 ms interruption | 100–500 ms every ~15 s ^29^| Affects streaming; mitigated by token-level streaming buffers |
| Download bandwidth | 100–400 Mbps ^1^| Shared among all users on vessel | Not a constraint for LLM API traffic |
| Upload bandwidth | 10–40 Mbps ^1^| Lower during congestion | Voice upload (Opus-encoded) is ~6–24 kbps; ample headroom |
| Packet loss | < 1 % ^1^| 2–10 % during heavy rain or obstruction | TCP retransmit handles loss; QUIC preferred for 0-RTT recovery |
| Cold-start boot | 2–5 minutes ^11^| N/A | Local LLM must handle voice commands during terminal boot |
...
Four techniques reduce the effective latency of LLM API calls over Starlink. First, the gateway maintains **persistent HTTP/2 connections** to OpenAI's API endpoint, eliminating repeated TLS handshakes that each add 100–300 ms on a satellite link ^29^. Second, a **connection pool** of 4–8 long-lived TCP connections handles concurrent voice sessions from different rooms, multiplexing requests over a single TLS context. Third, the Pi batches non-urgent telemetry and log uploads into larger payloads, reducing per-packet overhead and avoiding small-packet inefficiency on TCP over satellite. Fourth, simple commands ("lights on", "set throttle to 50 %") are routed to GPT-4o-mini rather than GPT-4o, cutting time-to-first-token from ~200 ms to ~100 ms ^14^. For streaming audio responses, the system uses WebSocket connections that tolerate the 100–500 ms handoff spikes by buffering 2–3 seconds of audio tokens ahead of playback. gRPC with Protobuf is used for service-to-service communication within the edge cluster, delivering 50–70 % lower latency and 5–10× smaller payloads than REST/JSON ^30^ ^31^, though the external LLM API calls remain HTTP/2 because the cloud providers do not expose gRPC endpoints for GPT-4o.
...
|-------|------------|---------------|----------|---------------------|--------------|
| Voice capture (I2S buffer) | 40 | 40 ^5^| 0 | INMP441 microphone + ESP32 I2S DMA | Reduce ring-buffer size from 40 ms to 20 ms |
| VAD endpointing (silence detection) | 500 | 650 ^5^| +150 | Silero VAD waiting for speech-end confirmation | Tune endpointing eagerness; use streaming VAD with 30 ms frames |
| STT (Whisper tiny.en, local) | 300 | 280 ^9^| -20 | whisper.cpp on Raspberry Pi 5 at 3.5x real-time | Streaming incremental decode (50–150 ms incremental) |
| LLM generation (cloud via Starlink) | 500 | 850 ^1^ ^14^| +350 | GPT-4o-mini TTFT over Starlink RTT + server queuing | Route simple commands to local Jetson (Llama 3.2 3B at ~28 tok/s) ^4^; use speculative TTS |
| TTS (Piper, local) | 100 | 120 ^5^ ^33^| +20 | Piper CPU synthesis on Raspberry Pi 5 | Cache 50 most-common phrases; pre-generate status responses |
| Audio playback (I2S output) | 40 | 40 ^5^| 0 | I2S DAC buffer + amplifier settle | Hardware-determined; minimal optimization headroom |
| **Total** | **1,480** | **1,980** | **+500** | **VAD + LLM = 87% of total gap** | See Section 10.2.3 |

The critical path analysis reveals that two stages — VAD endpointing and LLM generation — account for 87% of the 500 ms total gap against the allocated budget. VAD endpointing at 650 ms (budget: 500 ms) is dominated by the Silero VAD's conservative speech-end detection, which waits for sustained silence to avoid truncating commands in noisy maritime environments. LLM generation at 850 ms (budget: 500 ms) reflects the full round-trip over Starlink: 25–50 ms median RTT ^1^, 100–200 ms server time-to-first-token (TTFT) for GPT-4o-mini, and 600+ ms for token generation on longer responses ^14^.

Three optimization paths are available to close the gap. First, **streaming STT** enables incremental text output after the first 50–150 ms of audio processing, overlapping STT with VAD endpointing and reducing the effective serial latency by ~150 ms. Second, **speculative TTS** begins synthesizing the response from partial LLM output tokens rather than waiting for generation completion, an approach that can overlap 50–80% of TTS time with the tail of LLM generation. Third, routing simple commands (single-action intents such as "turn on lights" or "stop engine") to the local Jetson running Llama 3.2 3B at approximately 28 tok/s ^4^eliminates the Starlink round-trip entirely, reducing LLM stage latency from 850 ms to approximately 200–300 ms for short responses.
...
|--------|------|--------|---------------|--------|--------|
| Jetson Orin Nano 8GB | Llama 3.2 3B inference | tok/s (Q4_K_M) | ~28 tok/s ^4^| >25 tok/s | **Pass** |
| Jetson Orin Nano 8GB | Mistral 7B inference | tok/s (Q4_K_M) | ~17 tok/s ^4^| >15 tok/s | **Pass** |
| Jetson Orin Nano 8GB | Phi-3.5 Mini inference | tok/s (Q4_K_M) | ~25 tok/s ^4^| >22 tok/s | **Pass** |
| Jetson Orin Nano 8GB | Llama 3.2 3B RAM usage | GB allocated | 3.5 GB ^4^| <4.0 GB | **Pass** |
| Jetson Orin Nano 8GB | Mistral 7B RAM usage | GB allocated | 5.2 GB ^4^| <6.0 GB | **Pass** |
| ESP32-S3 | Wake-word inference | ms per window | 18–22 ms ^4^| <25 ms | **Pass** |
| ESP32-S3 | Model size | KB (INT8) | 80 KB ^4^| <100 KB | **Pass** |
| ESP32-S3 | Battery life (2000 mAh) | months listening | ~4 months ^6^| >3 months | **Pass** |
| Raspberry Pi 5 | whisper.cpp tiny.en | Real-time factor | 3.5x real-time ^9^| >2.0x | **Pass** |
| Raspberry Pi 5 | TinyLlama 1.1B inference | tok/s (Q4) | 12–18 tok/s ^8^| >10 tok/s | **Pass** |
| Raspberry Pi 5 | K3s orchestration | Minimum RAM | 512 MB ^10^| <1 GB | **Pass** |
| Starlink (LEO) | Median RTT | ms | 25–50 ms ^1^| <60 ms | **Pass** |
| Starlink (LEO) | 99th percentile RTT | ms | <65 ms ^1^| <100 ms | **Pass** |
| Starlink (LEO) | Packet loss (clear sky) | % | <1% ^1^| <2% | **Pass** |
| Starlink (LEO) | Download bandwidth | Mbps | 100–400 Mbps ^1^| >50 Mbps | **Pass** |

The Jetson Orin Nano results demonstrate that all three recommended local LLMs — Llama 3.2 3B, Mistral 7B, and Phi-3.5 Mini — execute within their 8 GB unified memory budget at quantization level Q4_K_M, with Llama 3.2 3B offering the highest throughput at approximately 28 tok/s while consuming only 3.5 GB RAM ^4^. This leaves sufficient headroom for concurrent TTS (Piper at ~4 GB VRAM ^36^) and STT (faster-whisper medium model) pipelines when the device operates in fully local offline mode. TensorRT-LLM compilation further improves throughput by 15–25% over llama.cpp baseline by fusing attention kernels and eliminating Python interpreter overhead in the hot path ^37^ ^38^.

The ESP32-S3 wake-word performance at 18–22 ms inference per 1-second audio window ^4^is achieved through a depthwise-separable CNN with four blocks, quantized to INT8 and operating on MFCC features extracted from 40 mel bins. Posterior smoothing over three consecutive windows with a threshold of 0.85 suppresses false activations at the cost of ~400 ms additional detection latency, an acceptable trade-off given the four-month battery life achieved on a 2000 mAh cell with deep-sleep duty cycling ^6^.

On the Raspberry Pi 5, whisper.cpp (tiny.en model) achieves 3.5x real-time transcription speed ^9^, meaning a 5-second utterance is transcribed in approximately 1.4 seconds — comfortably within the 300 ms STT budget when using streaming incremental decode. TinyLlama 1.1B at 12–18 tok/s ^8^is sufficient for simple command-and-control tasks when the Jetson is unavailable, though it falls below the quality threshold for multi-step reasoning. K3s runs reliably with a 512 MB RAM minimum ^10^, leaving the remaining 7.5 GB (on an 8 GB Pi) for Whisper, Ollama, Piper, and the MQTT broker.

Starlink's median RTT of 25–50 ms with 99th percentile under 65 ms ^1^makes it viable for real-time LLM API calls where total end-to-end budgets of 1–3 seconds are acceptable. Packet loss under clear conditions remains below 1% ^1^, and bandwidth at 100–400 Mbps far exceeds the requirements of voice AI traffic (an average STT request plus LLM response totals under 100 KB). The primary operational concern is satellite handoff events occurring approximately every 15 seconds, which can inject 100–500 ms latency spikes ^29^; mitigation via persistent HTTP/2 connections and local Jetson fallback during detected degradation keeps the 99th percentile voice-to-action latency under the 3,000 ms ceiling.

---

## 11. Implementation Roadmap

The preceding chapters defined a voice-controlled, self-assembling distributed system for maritime operation. This chapter converts that architecture into a twelve-month implementation plan, structured around five phases with defined deliverables, measurable exit criteria, and identified risk mitigations. The plan acknowledges four hard constraints from earlier analysis: a single contributor maintains 3,200+ repositories ^16^; no cross-repo integration test suite exists ^13^; the CopilotKit fork trails upstream by seven commits ^10^; and the hardware bill of materials totals approximately $1,200–1,600 one-time plus $75–120 per month for Starlink ^2^.

### 11.1 Development Phases

#### 11.1.1 Phase 1 — Foundation (Months 1–2)

Phase 1 establishes the minimal deployable unit: a single-node voice command pipeline on the Raspberry Pi 5 and Jetson Orin Nano pair, with one ESP32-S3 node in a single room. Engineering focus is on hardening the `construct-coordination` Rust runtime (80 commits ^20^), implementing the basic room model via PLATO SDK, closing the CopilotKit upstream gap, and creating a CI skeleton for the five load-bearing repositories. The exit criterion is a spoken command captured on the ESP32, processed through whisper.cpp STT at 3.5x real-time ^9^, routed to Llama 3.2 3B on the Jetson at ~28 tok/s ^4^, and answered via Piper TTS within the 3,000 ms P0 ceiling from Chapter 10.

#### 11.1.2 Phase 2 — Mesh (Months 3–4)

Phase 2 expands to multi-node coordination. The three-tier discovery protocol — mDNS for same-room, gossip (SIR epidemic model) for vessel-wide, Kademlia DHT for global — is implemented and stress-tested. ESP32 nodes deploy across all rooms (bridge, engine room, backdeck, hold), each running the 80 KB INT8 wake-word model at 18–22 ms inference ^4^. The four Vessels (Forgemaster, CCC, JetsonClaw1, Oracle1) register as DID-backed service identities with room-scoped capability tokens ^2^. Auto-join is verified empirically: 50 cold-boot cycles per tier must show a 90th-percentile join time below 60 seconds. This phase introduces the first cross-repo integration tests — the most significant technical blocker identified in Chapter 2 ^13^.
...
|---|---|---|---|---|---|
| 1 — Foundation | M1–M2 | Rust runtime; room model; single-node voice; CI skeleton | Hardware procured ($1,200–1,600 BOM) ^2^| Voice-to-action <3,000 ms (100 trials) | None |
...
| 3 — Intelligence | M5–M6 | NL pipeline optimization; Starlink LLM routing; connectivity state machine | Phase 2 exit; Starlink active ($75–120/mo) ^1^| Voice-to-action <2,000 ms hybrid path | Phase 2 |
...
| 5 — Optimization | M9–M12 | Full 5-layer pipeline; TensorRT-LLM tuning; benchmark achievement | Phase 4 exit; JetPack 6.1 on Orin Nano ^12^| All P0/P1 benchmarks from Chapter 10 passed | Phase 4 |
...
The five-layer compilation pipeline is the highest technical risk. The stack (`open-parallel` → `pincher` → `flux-core` → `cuda-oxide` → `cudaclaw`) has no validated end-to-end path; Chapter 2 found no automated test that natural language intent traverses all five layers and executes on GPU hardware ^13^. Latency feasibility is a second risk: the 500 ms gap identified in Chapter 10 concentrates in VAD endpointing (+150 ms) and LLM generation (+350 ms), requiring streaming implementations not yet in the codebase. The single-maintainer bottleneck is structural — one contributor across 3,200+ repositories yields a bus factor of one ^16^.
...
The capability-based security model with DID-backed Vessel identities ^2^has been specified but not penetration-tested; a compromised token could expose repository-scope capabilities. Consumer hardware (RPi 5, Jetson Orin Nano) is not marine-rated, and salt air, vibration, and thermal cycling present failure modes untested in the existing codebase. Documentation fragmentation — 1,500,000+ words across 2,000 repositories without unified indexing ^18^— makes incident response dependent on the single maintainer's institutional knowledge.
...
| R-TECH-03 | Single maintainer unavailable >2 weeks; bus factor of one | Medium | Critical | Document decisions in-repo via git-agent lifecycle ^27^; establish second contributor in Phases 1–2 | Project coordinator |
...
| R-INT-02 | LLM API costs exceed budget at multi-vessel scale | Medium | Medium | Route simple commands to local Jetson (~28 tok/s ^4^); cache 50 common responses; per-vessel rate limiting | Operations |
| R-INT-03 | Starlink outage exceeds 24-hour offline autonomy window | Low | High | 2,100 Wh LiFePO4 battery sustains 24+ h compute ^27^; reflex-only mode; command queue for replay | Marine engineer |
| R-OPS-01 | Capability token compromise exposes Vessel permissions | Low | Critical | 1-hour token expiry; room-scoped capabilities limit blast radius ^2^; gossip revocation; offline `did:key` validation | Security engineer |
| R-OPS-02 | Consumer hardware fails in marine environment | Medium | High | Conformal coating; active cooling (mandatory, prevents 20–30% thermal throttle ^8^); sealed enclosures; spares aboard | Hardware engineer |
| R-OPS-03 | Documentation fragmentation blocks contributor onboarding | High | Medium | Unified index from CATALOG.md ^17^; in-repo decision records required; automated doc-link CI check | Documentation lead |
...
The following table defines ten terms that carry precise, often SuperInstance-specific meanings derived from usage in preceding chapters and ecosystem documentation ^2^.
...
| **MUD** | Multi-User Dungeon — a spatial environment decomposed into discrete rooms with exits, contents, and triggers. SuperInstance adopts the MUD model as its universal application architecture ^20^. | Chapter 1 |
...
| **Pincher** | The Layer 2 reflex engine combining regex matching with vector database search for sub-millisecond response; 57 commits, 76.6% Rust ^24^. | Chapter 2 |
| **FLUX** | The intermediate representation (IR) for agent cognition at Layer 3. FLUX bytecode compiles intent to constraint programs; the transpiler targets 12 languages ^25^. | Chapter 2 |
...
| **CHARTER.md** | An agent repository file defining purpose, constraints, and behavioral boundaries. With `STATE` and `TASK-BOARD`, it constitutes the agent's runtime specification ^27^. | Chapter 2 |
...
|:---|:---|---:|---:|---:|:---|
| Microcontroller | ESP32-S3-DevKitC-1 | 6 | $12.50 | $75.00 | 240 MHz dual-core, 512 KB SRAM, 8 MB PSRAM ^7^|
| Microphone | INMP441 | 6 | $2.50 | $15.00 | I2S digital output; one per ESP32 node |
| Edge coordinator | Raspberry Pi 5 8 GB | 1 | $80.00 | $80.00 | Hosts K3s, Mosquitto, whisper.cpp ^10^|
| AI inference | Jetson Orin Nano 8 GB | 1 | $379.00 | $379.00 | 67 TOPS INT8, 1024 CUDA cores ^2^|
| Satellite terminal | Starlink Standard | 1 | $599.00 | $599.00 | 100–400 Mbps down, 25–50 ms RTT ^1^|
| Storage | 512 GB NVMe SSD (Pi HAT) | 1 | $45.00 | $45.00 | Reduces model load from ~18 s to ~3 s ^28^|
| Cooling | Pi 5 heatsink + fan | 1 | $15.00 | $15.00 | Prevents 20–30% thermal throttling ^8^|
| Power, enclosure, cables | Assorted | 1 | $75.00 | $75.00 | 12 V marine DC panel, IP65 enclosure, wiring |
| **One-time total** | | | | **$1,283** | |
| Satellite service | Starlink maritime | 1 | $97.50/mo | — | Range: $75–120/month ^1^|
...
**Mobile agent platforms.** The strong versus weak mobility distinction was established by Aglets (IBM, 1996–2005), Voyager (ObjectSpace, 1997), and the MOA program (Milojicic et al., USENIX COOTS 1998) ^28^. SuperInstance adopts weak mobility with structured state bundles, following the historical convergence of production platforms toward weak mobility under platform heterogeneity constraints. JAMES (MATA 1999) demonstrated 87% migration speedup via code prefetching, informing WASM preloading during room transitions.

**Self-organising coordination.** Beal et al.'s SCR provides the theoretical basis for room-based consensus with overlapping partitions ^24^. Li et al.'s SwarmSys (arXiv:2510.10047, 2024) achieves 10.7% higher accuracy than GPTSwarm via pheromone-inspired reinforcement; its explorer/worker/validator roles map to Vessel specialization. MoA-Off (Yang et al., arXiv:2509.16995, 2025) demonstrates 30% latency reduction through per-modality adaptive offloading, providing the edge-cloud LLM routing logic ^13^.

**Consensus and security.** Castro and Liskov's PBFT (OSDI 1999) underlies the ternary voting protocol. Fotiou et al.'s capability-based access control using Verifiable Credentials (SafeThings 2022) binds DID-backed identities to room-scoped permissions ^3^. Ongaro and Ousterhout's Raft (USENIX ATC 2014) operates within each physical room's trusted cluster.

**Industry systems.** CopilotKit provides the React hooks and AG-UI protocol mapping voice to tool calls ^2^. Assessed at 9/10 for agentic UI but 5/10 for distributed orchestration, it defines the boundary between supplied functionality and SuperInstance extensions. NVIDIA Isaac provides sensor-fusion patterns, though SuperInstance replaces its centralized orchestration with decentralized Vessel coordination. K3s runs on the Raspberry Pi 5 at 512 MB RAM minimum ^10^; Ollama manages local LLM loading on the Jetson ^4^. Starlink's LEO constellation at ~550 km altitude delivers 25–50 ms median RTT, roughly 10× lower than geostationary links ^1^. This makes SuperInstance, to our knowledge, the first distributed agent system explicitly designed around LEO-satellite latency as a core architectural parameter.
